...
# ========= Backtest =========
def _clip_by_dates(df, p):
    """Returnera df klippt till [from_date, to_date] om de finns i p."""
    import pandas as pd
    start = getattr(p, "from_date", None) if not isinstance(p, dict) else p.get("from_date")
    end   = getattr(p, "to_date",   None) if not isinstance(p, dict) else p.get("to_date")
    if not start and not end:
        return df
    x = df.copy()
    # Se till att vi kan jämföra mot en datetime-index/kolumn
    if "Date" in getattr(x, "columns", []):
        x["Date"] = pd.to_datetime(x["Date"], errors="coerce")
        idx = x["Date"]
        use_index = False
    else:
        try:
            ...
            return df  # kan inte klippa på ett säkert sätt
...
def run_backtest(df: pd.DataFrame, p: Params) -> Dict[str, Any]:
    # Coerce dict -> Params so we can access attributes like p.rsi_window
    if isinstance(p, dict):
        try:
            # bara nycklar som finns i Params-annoteringarna
            keys = getattr(Params, '__annotations__', {}).keys()
            filtered = {k: v for k, v in p.items() if k in (keys or [])}
            p = Params(**filtered)
        except Exception as e:
            raise TypeError(f'Could not build Params from dict: {e}')

    df = _fix_df_source(df, (p if isinstance(p, dict) else (vars(p) if hasattr(p, "__dict__") else None)))

    x = _normalize_df(df)
    x = _clip_by_dates(x, p)
...
def _fix_df_source(df, params):
    """If df is a ticker string, fetch OHLCV. params may be dict or object."""
    prof = params if isinstance(params, dict) else (vars(params) if hasattr(params, "__dict__") else {})
    if isinstance(df, str):
        s = df.strip()
        # Heuristik: kort sträng med bokstäver/siffror => sannolikt ticker
        if s and any(c.isalpha() for c in s) and len(s) <= 32:
            ticker = s
            start = prof.get("from_date")
            end = prof.get("to_date")
            try:
