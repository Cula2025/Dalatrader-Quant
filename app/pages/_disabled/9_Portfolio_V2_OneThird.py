...
st.set_page_config(page_title="Portfolio V2 – 1/3-regel", layout="wide")
st.title("📈 Portfolio V2 – 1/3-regel (ingen belåning)")
...
# Plocka bästa profilen i varje fil (högst TotalReturn), kör backtest och hämta signaler
signals = []
tickers = []
start_dates = []
...
for f in sel_files:
    ...
    st.caption(f"**{ticker}** – använder profil: *{p.get('name')}*")
...
# Priser för alla tickers + OMXS30GI
...
c1.metric("Portfolio", f"{m['Portfolio_final']:.2f}×" if m["Portfolio_final"] else "—")
c2.metric("Buy & Hold", f"{m['BuyHold_final']:.2f}×" if m["BuyHold_final"] else "—")
c3.metric("OMXS30GI", f"{m['Index_final']:.2f}×" if m["Index_final"] else "—")

with st.expander("📒 Utförda affärer (simulerade med 1/3-regel)"):
    if not ledger.empty:
        st.dataframe(ledger.reset_index().rename(columns={"index":"date"}))
    else:
        st.write("Inga affärer.")
