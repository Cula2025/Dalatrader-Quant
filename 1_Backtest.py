...
def _collect_params_from_state():
    ...
    if not params.get("from_date") and "start" in s:
        params["from_date"] = st.session_state["start"]
    if not params.get("to_date") and "end" in s:
        params["to_date"] = st.session_state["end"]
    return params
...
with col_left:
    ...
    # REMOVED_TOPBLOCK: st.text_input("Ticker", key="ticker", value=tkr)  # visar vad som ligger i state
...
with col_right:
    try:
        ...
        if ready and (do_run or st.session_state.get("__autorun__", True)):
            ...
            # REMOVED_TOPBLOCK: Resultat-rubrik
            st.write({
...
def _dt_collect_params_for_bt():
    ...
    return {k: st.session_state.get(k) for k in keys if k in s}

def _dt_param_form():
    s = _st.session_state
    _st.subheader("Parametrar (ändringsbara)")

# --- dalatrader: 4-col params begin ---
# 4-spaltslayout för parametrar (unika UI-keys) + synk till kanoniska keys
import streamlit as _st

def __dt_get(name, default=None):
    return _st.session_state.get(name, default)

with _st.container():
    c1, c2, c3, c4 = _st.columns(4, gap="small")

    # --- kolumn 1: datum, ticker, RSI ---
    with c1:
        _st.text_input("Ticker", value=__dt_get("ticker",""), key="__ui_ticker")
        _st.text_input("Från (YYYY-MM-DD)", value=__dt_get("from_date",""), key="__ui_from_date")
        _st.text_input("Till (YYYY-MM-DD)", value=__dt_get("to_date",""), key="__ui_to_date")
        _st.checkbox("RSI-filter", value=bool(__dt_get("use_rsi_filter", False)), key="__ui_use_rsi_filter")
        _st.number_input("RSI-fönster", min_value=1, step=1, value=int(__dt_get("rsi_window", 14) or 14), key="__ui_rsi_window")
        _st.number_input("RSI min", step=0.01, value=float(__dt_get("rsi_min", 30.0) or 30.0), format="%.2f", key="__ui_rsi_min")
        _st.number_input("RSI max", step=0.01, value=float(__dt_get("rsi_max", 70.0) or 70.0), format="%.2f", key="__ui_rsi_max")

    # --- kolumn 2: breakout/exit + MACD ---
    with c2:
        _st.number_input("Breakout lookback", min_value=1, step=1,
                         value=int(__dt_get("breakout_lookback", 20) or 20), key="__ui_breakout_lookback")
        _st.number_input("Exit lookback", min_value=1, step=1,
                         value=int(__dt_get("exit_lookback", 10) or 10), key="__ui_exit_lookback")

        _st.checkbox("MACD-filter", value=bool(__dt_get("use_macd_filter", False)), key="__ui_use_macd_filter")
        _st.number_input("MACD fast", min_value=1, step=1,
                         value=int(__dt_get("macd_fast", 12) or 12), key="__ui_macd_fast")
        _st.number_input("MACD slow", min_value=1, step=1,
                         value=int(__dt_get("macd_slow", 26) or 26), key="__ui_macd_slow")
        _st.number_input("MACD signal", min_value=1, step=1,
                         value=int(__dt_get("macd_signal", 9) or 9), key="__ui_macd_signal")

    # --- kolumn 3: trend + Bollinger ---
    with c3:
        _st.checkbox("Trendfilter", value=bool(__dt_get("use_trend_filter", False)), key="__ui_use_trend_filter")
        _st.selectbox("MA-typ", ["SMA","EMA"], index=0 if (__dt_get("trend_ma_type","SMA")=="SMA") else 1, key="__ui_trend_ma_type")
        _st.number_input("Trend MA-fönster", min_value=1, step=1,
                         value=int(__dt_get("trend_ma_window", 200) or 200), key="__ui_trend_ma_window")

        _st.checkbox("Bollinger-filter", value=bool(__dt_get("use_bb_filter", False)), key="__ui_use_bb_filter")
        _st.number_input("BB window", min_value=1, step=1,
                         value=int(__dt_get("bb_window", 20) or 20), key="__ui_bb_window")
        _st.number_input("BB n std", min_value=0.1, step=0.01,
                         value=float(__dt_get("bb_nstd", 2.0) or 2.0), format="%.2f", key="__ui_bb_nstd")
        _st.number_input("BB min", min_value=0.0, step=0.01,
                         value=float(__dt_get("bb_min", 0.25) or 0.25), format="%.2f", key="__ui_bb_min")

    # --- kolumn 4: stop/ATR ---
    with c4:
        _st.checkbox("Stop-loss", value=bool(__dt_get("use_stop_loss", False)), key="__ui_use_stop_loss")
        _st.selectbox("Stop mode", ["pct","atr"],
                      index=0 if (__dt_get("stop_mode","pct")=="pct") else 1, key="__ui_stop_mode")
        _st.number_input("Stop loss % (0–1)", min_value=0.0, max_value=1.0, step=0.01,
                         value=float(__dt_get("stop_loss_pct", 0.05) or 0.05), format="%.2f", key="__ui_stop_loss_pct")

        _st.number_input("ATR window", min_value=1, step=1,
                         value=int(__dt_get("atr_window", 20) or 20), key="__ui_atr_window")
        _st.number_input("ATR mult", min_value=0.1, step=0.01,
                         value=float(__dt_get("atr_mult", 2.0) or 2.0), format="%.2f", key="__ui_atr_mult")

        _st.checkbox("ATR trailing", value=bool(__dt_get("use_atr_trailing", False)), key="__ui_use_atr_trailing")
        _st.number_input("ATR trail mult", min_value=0.1, step=0.01,
                         value=float(__dt_get("atr_trail_mult", 1.5) or 1.5), format="%.2f", key="__ui_atr_trail_mult")

# --- synk: kopiera UI-keys -> kanoniska keys i session_state ---
__map = {
 "ticker":"__ui_ticker",
 "from_date":"__ui_from_date",
 "to_date":"__ui_to_date",
 "use_rsi_filter":"__ui_use_rsi_filter",
 "rsi_window":"__ui_rsi_window",
 "rsi_min":"__ui_rsi_min",
 "rsi_max":"__ui_rsi_max",
 "breakout_lookback":"__ui_breakout_lookback",
 "exit_lookback":"__ui_exit_lookback",
 "use_macd_filter":"__ui_use_macd_filter",
 "macd_fast":"__ui_macd_fast",
 "macd_slow":"__ui_macd_slow",
 "macd_signal":"__ui_macd_signal",
 "use_trend_filter":"__ui_use_trend_filter",
 "trend_ma_type":"__ui_trend_ma_type",
 "trend_ma_window":"__ui_trend_ma_window",
 "use_bb_filter":"__ui_use_bb_filter",
 "bb_window":"__ui_bb_window",
 "bb_nstd":"__ui_bb_nstd",
 "bb_min":"__ui_bb_min",
 "use_stop_loss":"__ui_use_stop_loss",
 "stop_mode":"__ui_stop_mode",
 "stop_loss_pct":"__ui_stop_loss_pct",
 "atr_window":"__ui_atr_window",
 "atr_mult":"__ui_atr_mult",
 "use_atr_trailing":"__ui_use_atr_trailing",
 "atr_trail_mult":"__ui_atr_trail_mult",
}
for dst, src_key in __map.items():
    if src_key in _st.session_state:
        _st.session_state[dst] = _st.session_state[src_key]
# --- dalatrader: 4-col params end ---
    ...
    with col1:
        ...
        _st.text_input("Från (YYYY-MM-DD)", key="bt_from_date")
        _st.text_input("Till (YYYY-MM-DD)", key="bt_to_date")

        _st.checkbox("RSI-filter", key="use_rsi_filter", value=bool(st.session_state.get("use_rsi_filter", False)))
        _st.number_input("RSI-fönster", min_value=1, value=int(st.session_state.get("rsi_window", 14)), step=1, key="rsi_window")
        _st.number_input("RSI min", value=float(st.session_state.get("rsi_min", 30.0)), key="rsi_min")
        _st.number_input("RSI max", value=float(st.session_state.get("rsi_max", 70.0)), key="rsi_max")

        _st.checkbox("Trendfilter", key="use_trend_filter", value=bool(st.session_state.get("use_trend_filter", False)))
        _st.selectbox("MA-typ", options=["SMA","EMA"],
                      index=(0 if st.session_state.get("trend_ma_type","SMA")=="SMA" else 1),
                      key="trend_ma_type")
        _st.number_input("Trend MA-fönster", min_value=1, value=int(st.session_state.get("trend_ma_window",200)), step=1, key="trend_ma_window")

        _st.checkbox("Bollinger-filter", key="use_bb_filter", value=bool(st.session_state.get("use_bb_filter", False)))
        _st.number_input("BB window", min_value=1, value=int(st.session_state.get("bb_window",20)), step=1, key="bb_window")
        _st.number_input("BB n std", value=float(st.session_state.get("bb_nstd",2.0)), key="bb_nstd")
        _st.number_input("BB min", value=float(st.session_state.get("bb_min",0.0)), key="bb_min")

    # ---- höger
    with col2:
        _st.number_input("Breakout lookback", min_value=1, value=int(st.session_state.get("breakout_lookback",55)), step=1, key="breakout_lookback")
        _st.number_input("Exit lookback", min_value=1, value=int(st.session_state.get("exit_lookback",20)), step=1, key="exit_lookback")

        _st.checkbox("MACD-filter", key="use_macd_filter", value=bool(st.session_state.get("use_macd_filter", False)))
        _st.number_input("MACD fast", min_value=1, value=int(st.session_state.get("macd_fast",12)), step=1, key="macd_fast")
        _st.number_input("MACD slow", min_value=1, value=int(st.session_state.get("macd_slow",26)), step=1, key="macd_slow")
        _st.number_input("MACD signal", min_value=1, value=int(st.session_state.get("macd_signal",9)), step=1, key="macd_signal")

        _st.checkbox("Stop-loss", key="use_stop_loss", value=bool(st.session_state.get("use_stop_loss", False)))
        _st.selectbox("Stop mode", options=["pct","atr"],
                      index=(0 if st.session_state.get("stop_mode","pct")=="pct" else 1),
                      key="stop_mode")
        _st.number_input("Stop loss % (0–1)", min_value=0.0, max_value=1.0, value=float(st.session_state.get("stop_loss_pct",0.1)), key="stop_loss_pct")

        _st.number_input("ATR window", min_value=1, value=int(st.session_state.get("atr_window",14)), step=1, key="atr_window")
        _st.number_input("ATR mult", value=float(st.session_state.get("atr_mult",2.0)), key="atr_mult")
        _st.checkbox("ATR trailing", key="use_atr_trailing", value=bool(st.session_state.get("use_atr_trailing", False)))
        _st.number_input("ATR trail mult", value=float(st.session_state.get("atr_trail_mult",2.0)), key="atr_trail_mult")
...
def _dt_actions():
    ...
    # Kör backtest
    if colA.button("▶ Kör backtest", use_container_width=True):
        ticker = (st.session_state.get("ticker") or "").strip()
        params = _dt_collect_params_for_bt()
        payload = {"ticker": ticker, "params": params}
        try:
            res = _RUNBT(payload)
            st.session_state["__bt_last_res__"] = res
            summ = res.get("summary", {})
    ...
    # Spara profil
    save_name_default = (st.session_state.get("loaded_profile_name") or st.session_state.get("ticker") or "Profil")
    save_name = colB.text_input("Profilnamn att spara", value=f"{save_name_default}".strip(), key="__save_name__")
    if colB.button("💾 Spara profil", use_container_width=True):
        try:
            base = _Path("profiles"); base.mkdir(exist_ok=True)
            ticker = (st.session_state.get("ticker") or "TICKER").strip()
            ...
            prof = {
                ...
                "ticker": ticker,
                "params": _dt_collect_params_for_bt(),
                "metrics": (st.session_state.get("__bt_last_res__", {}) or {}).get("summary", {})
            }
...
def _dt_collect_params_from_ui():
    s = st.session_state
    params = {
        "from_date": st.session_state.get("bt_from") or None,
        "to_date":   st.session_state.get("bt_to") or None,

        "use_rsi_filter": st.session_state.get("bt_use_rsi", False),
        "rsi_window":     st.session_state.get("bt_rsi_window", 14),
        "rsi_min":        st.session_state.get("bt_rsi_min", 30.0),
        "rsi_max":        st.session_state.get("bt_rsi_max", 70.0),

        # Lägg till fler parametrar här om de finns i din UI (MACD, trend, stop-loss, BB, ATR...)
        # Exempel:
        "use_macd_filter": st.session_state.get("bt_use_macd", False),
        "macd_fast":       st.session_state.get("bt_macd_fast", 12),
        "macd_slow":       st.session_state.get("bt_macd_slow", 26),
        "macd_signal":     st.session_state.get("bt_macd_signal", 9),

        "use_stop_loss":   st.session_state.get("bt_use_sl", False),
        "stop_mode":       st.session_state.get("bt_stop_mode", "pct"),
        "stop_loss_pct":   st.session_state.get("bt_sl_pct", 0.05),

        "atr_window":      st.session_state.get("bt_atr_win", 14),
        "atr_mult":        st.session_state.get("bt_atr_mult", 2.0),
        "use_atr_trailing":st.session_state.get("bt_atr_trail", False),
        "atr_trail_mult":  st.session_state.get("bt_atr_trail_mult", 1.5),

        "use_trend_filter":st.session_state.get("bt_use_trend", False),
        "trend_ma_type":   st.session_state.get("bt_trend_ma_type", "SMA"),
        "trend_ma_window": st.session_state.get("bt_trend_ma_win", 200),

        "use_bb_filter":   st.session_state.get("bt_use_bb", False),
        "bb_window":       st.session_state.get("bt_bb_win", 20),
        "bb_nstd":         st.session_state.get("bt_bb_nstd", 2.0),
        "bb_min":          st.session_state.get("bt_bb_min", 0.25),

        "breakout_lookback": st.session_state.get("bt_brk_lb", 26),
        "exit_lookback":     st.session_state.get("bt_exit_lb", 15),
    }
    return params
...
def _dt_render_backtest_ui():
    ...
    with col_params:
        ...
        run = st.button("Kör backtest", type="primary", use_container_width=True, key="bt_run")
        ...
        if payload and payload.get("ticker"):
            try:
                ...
                if trades_df is not None and len(trades_df):
                    # UI_SUPPRESS: st.dataframe(trades_df, use_container_width=True, hide_index=True)
